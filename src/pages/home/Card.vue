<template>
    <el-card class="box-card" shadow="hover" @click="goDetail">
      <div class="content">
        <div class="left">
          <div class="hospital_name">{{hospialInfo.hosname}}</div>
          <div class="tip">
            <div class="level">
              <svg
                t="1684986163807"
              ...
              <span>{{hospialInfo.param.hostypeSteing}}</span>
            </div>
            <div class="time">
              <svg
                t="1684986208058"
              ...
              <span>每天{{hospialInfo.bookingRule?.releaseTime}}</span>
            </div>
          </div>
        </div>
        <div class="right">
          <img :src="`data:image/jpeg;base64,${hospialInfo.logoData}`" alt="" />
        </div>
      </div>
    </el-card>
  </template>
...
//接受父组件Home传递过来的props
let props=defineProps(['hospialInfo'])
let router=useRouter()
//点击卡片跳转到详情页
const goDetail=()=>{
  router.push({
    path:'/hospital/register',
    query:{hoscode:props.hospialInfo.hoscode}
  })
}
  </script>
